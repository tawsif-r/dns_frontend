...
function CricketLiveDashboard() {
  ...
  const [errorEventTypes, setErrorEventTypes] = useState(null);
  ...
  useEffect(() => {
    const fetchEventTypes = async () => {
      try {
        ...
        const baseUrl = 'http://192.168.3.35:8002/sport/api/events/?sport_category=cricket';
        const response = await axios.get(baseUrl);
        console.log('Event Types API Response:', response.data);
        const activeEvents = response.data.filter((event) => event.active);
        setEventTypes(activeEvents);
      } catch (error) {
        console.error('Error fetching Event Types:', error);
        setErrorEventTypes('Failed to load event types. Please try again later.');
        setEventTypes([]);
      } finally {
  ...
  const handleGenerateMessage = async () => {
    ...
    if (!selectedEventType) {
      setGenerateStatus({ type: 'error', message: 'Please select an event type.' });
      return;
    }
    ...
    try {
      const eventTypeName = eventTypes.find((event) => event.id === parseInt(selectedEventType))?.event_type || '';
      const fullMessage = `${eventTypeName}! ${messageInput}`;
      const payload = {
        text_for_review: fullMessage,
        ...
        <div className="flex-1 p-6">
          ...
          <div className="bg-gray-800 rounded-lg p-6">
            ...
            <div className="mb-4">
              ...
              {loadingEventTypes ? (
                <div className="text-xs text-gray-400">Loading event types...</div>
              ) : errorEventTypes ? (
                <div className="text-xs text-red-400">{errorEventTypes}</div>
              ) : eventTypes.length === 0 ? (
                <div className="text-xs text-gray-400">No event types available.</div>
              ) : (
                <select
            ...
            <div className="mb-4">
              ...
              <textarea
                placeholder="Enter your message (e.g., Jadeja bowls Iyer for 34!)"
                className="w-full bg-gray-700 border border-gray-600 rounded px-3 py-2 text-sm focus:outline-none focus:border-blue-500"
                rows="4"
                value={messageInput}
            ...
            <button
              ...
              disabled={generating || !messageInput.trim() || !selectedEventType}
              className={`w-full py-2 rounded-lg text-sm font-semibold transition-colors ${
                generating || !messageInput.trim() || !selectedEventType
